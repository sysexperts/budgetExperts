...
function createWindow() {
  ...
  // Erstelle eine korrigierte HTML-Datei
  const indexPath = path.join(__dirname, 'dist', 'index.html');
  let htmlContent = fs.readFileSync(indexPath, 'utf8');
  
  // Korrigiere alle Pfade von absolut zu relativ
  htmlContent = htmlContent.replace(/src="\/assets\//g, 'src="./assets/');
  htmlContent = htmlContent.replace(/href="\/assets\//g, 'href="./assets/');
  
  // Ersetze auch vite.svg mit relativem Pfad
  htmlContent = htmlContent.replace(/href="\/vite.svg"/g, 'href="./vite.svg"');
  
  // Schreibe die korrigierte HTML-Datei
  const correctedIndexPath = path.join(__dirname, 'dist', 'index_fixed.html');
